/**
 * @file canvas-storage.ts
 * @description Zustand store for canvas state management with localStorage persistence
 * Ported from analytics-template for GenUI Terminal mode
 */
    ...
    persist(
        (set, get) => ({
            ...
            createCanvas: (name?: string) => {
                ...
                const canvasName = name || `Canvas ${canvases.length + 1}`;
            ...
            // Copy component (user requested copy behavior, not move)
            copyComponent: (
                sourceCanvasId: string,
                targetCanvasId: string,
                componentId: string
            ) => {
                const operationKey = `copy-${componentId}-${sourceCanvasId}-${targetCanvasId}`;
                const pendingOps = get().pendingOperations;
                ...
                let copiedComponent: CanvasComponent | null = null;
                ...
                set((state) => {
                    ...
                    const updatedCanvases = state.canvases.map((c) => {
                        if (c.id === targetCanvasId) {
                            return {
                                ...c,
                                components: [...c.components, copiedComponent!],
                            };
                        }
                        return c;
                    });
